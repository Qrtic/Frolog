// Taken from http://stackoverflow.com/questions/3326512/f-priority-queue
...
module PriorityQueue =
  ...
  let rec getAll =
    ...
    | HeapNotEmpty(min,h) ->
        seq {
            yield min
            let rec folder (st: 'a treeElement list) (Node(_, el, l)) =
                (el::st)@(List.fold folder [] l)
            yield! (h |> List.fold folder [])
        }
