...
module Search =
    ...
    let process_predicate(c: context) (s: signature) (comp: predicate): bool * context =
        ...
        let res: result =
            if not (check_parameters_count s comp) then Rejected
                ...
                let prms = s.parameters |> List.map (convert_to_arg c)
                match comp with
                    | F0(f0) -> f0
                    | F1(f1) -> f1 prms.Head
            ...
            for r in acceptedRules do
                match r with
                    ...
                    | ConcatenatedRule(_, p, next_r) -> 
                        let proc = process_predicate c s p
                        if fst proc then
                            let next = find d (snd proc) next_r.Signature
                            for proced in next do
                                yield proced
        }

    let exec (d: knowledgebase) (start: context) (s: signature): unit =
        printfn "Execute: %s. Context = %s" s.ToStr (start.ToString())
    ...
    let test = exec testkb Map.empty {name = "f"; parameters = ["D"]}